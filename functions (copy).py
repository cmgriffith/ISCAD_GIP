...
def FEMM_createmodel(parameters):
    ...
    for slot in range(0,Qs): # for each slot Qs
        for n in range(0,len(R_slot)): # for each coordinate n
            femm.mi_addnode(x_s[slot,n],y_s[slot,n]) # add node at relevant coordinate
        for n in range(0,len(R_slot)-1):
            femm.mi_addsegment( x_s[slot,n] , y_s[slot,n] , x_s[slot,n+1] , y_s[slot,n+1] )
    ...
    Th_ri = np.atan( (w_bar/2) / (r_r - h_bar) ) # inner half pitch angle
    Th_ro = np.atan( (w_bar/2) / r_r ) # outer half pitch angle
    # polar coordinates arrays for bar nodes
    R_bar = np.array([r_r,r_r - h_bar,r_r - h_bar,r_r])
    ...
    for bar in range(0,Qr): # for each slot Qs
        for n in range(0,len(R_bar)): # for each coordinate n
            femm.mi_addnode(x_r[bar,n],y_r[bar,n]) # add node at relevant coordinate
        for n in range(0,len(R_bar)-1):
            femm.mi_addsegment( x_r[bar,n] , y_r[bar,n] , x_r[bar,n+1] , y_r[bar,n+1] )
        # get coil region centre coords
