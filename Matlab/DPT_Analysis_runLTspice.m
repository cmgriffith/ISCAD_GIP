...
gate_resistor          = R_G;      % Ω  — gate input resistor
...
gate_inductance        = 20e-9;    % H  — gate input inductance
power_loop_inductance  = 3.8e-9;   % H
common_mode_inductance = 1e-9;     % H
DC_link_inductance     = 5.65e-9;  % H
branch_inductance      = 12e-9;    % H  — bus bar inductance between parallel branches
...
branch_resistance      = 20e-6;    % Ω  — bus bar resistance between parallel branches


%% Program Variables
sim_timeoffset = 10e-6; % LTspice allocates 10us before T1 begins
timewindow = 1150;
colors = {[0 0.447 0.741], [0.850 0.325 0.098], [0.466 0.674 0.188]}; % Consistent MOSFET colours: blue / orange-red / green
...
%% Section 4: Run LTspice simulation

spice_params = struct( ...
    'gate_resistor',          gate_resistor, ...
    'snubber_capacitor',      snubber_capacitor, ...
    'snubber_resistor',       snubber_resistor, ...
    'DC_link_capacitor',      DC_link_capacitor, ...
    'gate_inductance',        gate_inductance, ...
    'power_loop_inductance',  power_loop_inductance, ...
    'common_mode_inductance', common_mode_inductance, ...
    'DC_link_inductance',     DC_link_inductance, ...
    'branch_inductance',      branch_inductance, ...
    'DC_link_resistance',     DC_link_resistance, ...
    'C_DC_ESR',               C_DC_ESR, ...
    'C_snb_ESR',              C_snb_ESR, ...
    'branch_resistance',      branch_resistance);

disp('writing LTspice scratch file...')
write_ltspice_scratch(asc_file, scratch_asc, spice_params);

disp('running LTspice sim...')
system(sprintf('"%s" -b -ascii "%s"', ltspice_exe, scratch_asc));

%% Section 4b: Import simulation results

disp('importing LTspice sim data...')
sim = read_ltspice_raw(scratch_raw);

% Raw file stores single-ended node voltages; differential pairs computed here
time_sim      = sim('time')          - sim_timeoffset;
VDS1_sim      = sim('V(phase)')      - sim('V(v_s_low1)');   % V(phase,V_S_low1)
VDS2_sim      = sim('V(n016)')       - sim('V(v_s_low2)');   % V(N016,V_S_low2)
VDS3_sim      = sim('V(n018)')       - sim('V(v_s_low3)');   % V(N018,V_S_low3)
PWLdriver_sim = sim('V(n022)')       - sim('V(low)');         % V(N022,low)
VGS1_sim      = sim('V(v_g_low1)')  - sim('V(v_s_low1)');   % V(V_G_low1,V_S_low1)
VGS2_sim      = sim('V(v_g_low2)')  - sim('V(v_s_low2)');   % V(V_G_low2,V_S_low2)
VGS3_sim      = sim('V(v_g_low3)')  - sim('V(v_s_low3)');   % V(V_G_low3,V_S_low3)
I_total_sim   = sim('I(L2)');
ID1_sim       = sim('I(L_cm1[L])');
ID2_sim       = sim('I(L_cm2[L])');
ID3_sim       = sim('I(L_cm3[L])');
I_load_sim    = sim('I(L_ph)');
IG1_sim       = sim('I(R_g1)');
IG2_sim       = sim('I(R_g2)');
IG3_sim       = sim('I(R_g3)');
...
% commented ';.param' lines are left untouched.
    ...
    for k = 1:numel(names)
        name    = names{k};
        val_str = sprintf('%.6g', params.(name));   % e.g. 3.8e-09, 4.7, 99e-09
        % Match: !.param <name> = <old_value>   (to end of line)
